    ...
    public function processTindak($id_dokumenDO)
    {
        ...
        if (!empty($file_1)) {
            if ($this->upload->do_upload('ud_pprDO')) {
                ...
                $dataOne['ud_pprDO'] = 'UNDANGAN_PEMAPARAN'.$uniqueFile.'.'.$getFileOne['extension'];
                rename($data_file['full_path'], './public/lampiran/'.$dataOne['ud_pprDO']);
            }
        }
